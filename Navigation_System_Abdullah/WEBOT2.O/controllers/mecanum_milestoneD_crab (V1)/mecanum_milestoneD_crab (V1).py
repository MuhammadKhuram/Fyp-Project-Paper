...
ROW_SWITCH_MODE = "crab"   # HARDCODED — this file is crab-mode only
CRAB_SPEED = 0.3            # m/s lateral speed during crab strafe
...
while robot.step(TIME_STEP) != -1:
    ...
    # --- Navigation: crab mode for row-switch segments, normal controller otherwise ---
    if is_row_switch_target(current_wp_idx):
        # Crab mode: pure lateral strafe, heading held constant, no forward creep.
        vy_world = CRAB_SPEED if dy > 0 else -CRAB_SPEED
        vx_body = -vy_world * math.sin(yaw)
        vy_body = -vy_world * math.cos(yaw)

        heading_hold_error = switch_start_heading - yaw
        heading_hold_error = math.atan2(math.sin(heading_hold_error), math.cos(heading_hold_error))
        ...
        # Existing heading-correction controller for all normal in-row driving.
