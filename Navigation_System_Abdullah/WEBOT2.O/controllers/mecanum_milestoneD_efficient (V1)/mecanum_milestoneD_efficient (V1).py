...
ROW_SWITCH_MODE = "efficient"  # HARDCODED — this file is the no-fixed-front variant
CRAB_SPEED = 0.3                # m/s lateral speed during crab strafe (row switches only)
HEADING_HOLD_K = 1.0            # gain to resist yaw drift during crab strafe
...
while robot.step(TIME_STEP) != -1:
        ...
        if current_wp_idx == len(waypoints):
            ...
            with open(row_switch_path, "w", newline="") as f:
                ...
                writer.writerow(["mode", "switch_index", "start_time_s", "end_time_s",
                                  "switch_time_s", "energy_estimate", "entry_heading_deg"])
    ...
    if current_wp_idx != last_seen_wp_idx:
        ...
        if is_row_switch_target(current_wp_idx):
            ...
            switch_start_heading = yaw   # this is the value we now log directly, no derived "error"
            switch_energy = 0.0
            switch_count += 1
        else:
            switch_active = False
    ...
    if dist < REACH_THRESHOLD:
        ...
        if switch_active:
            ...
            row_switch_log.append((
                ...
                math.degrees(wrap_angle(switch_start_heading))   # entry heading, wrapped to [-180, 180]
            ))
            switch_active = False
    ...
    # --- Navigation ---
    if is_row_switch_target(current_wp_idx):
        # Row switch: crab strafe, heading held constant (identical to validated crab mode).
        # No rotate phase — this is the lateral-only move, direction/front concept
        # doesn't apply here since motion is purely sideways.
        vy_world = CRAB_SPEED if dy > 0 else -CRAB_SPEED
        vx_body = -vy_world * math.sin(yaw)
        vy_body = -vy_world * math.cos(yaw)

        heading_hold_error = wrap_angle(switch_start_heading - yaw)
        omega = max(-OMEGA_MAX, min(OMEGA_MAX, HEADING_HOLD_K * heading_hold_error))

        cmd_speeds = set_mecanum_velocity(vx_body, vy_body, omega)
        forward_speed = vx_body
        heading_error = heading_hold_error  # for debug print only
    else:
        # In-row driving: no fixed front/rear. Compare the turn needed to face the
        # target using the CURRENT heading vs. the current heading + 180 deg, and
        # drive forward or in reverse accordingly, whichever needs less rotation.
        # This is what removes the ~100-180 deg re-turns seen at the start of every
        # stripe in turn mode -- the robot simply reverses instead of spinning around.
        raw_target_heading = math.atan2(dy, dx)
        err_forward = wrap_angle(raw_target_heading - yaw)
        err_reverse = wrap_angle(err_forward - math.pi)

        if abs(err_forward) <= abs(err_reverse):
            direction_sign = 1.0
            heading_error = err_forward
        else:
            direction_sign = -1.0
            heading_error = err_reverse
        ...
        forward_speed = direction_sign * min(speed_cap, dist) * max(0.0, math.cos(heading_error))
