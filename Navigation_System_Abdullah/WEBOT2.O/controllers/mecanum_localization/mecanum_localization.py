...
CRUISE_SPEED = 0.6          # cap on straights (heading error small)
STRAIGHT_HEADING_THRESHOLD = math.radians(5)  # below this, treat as "straight enough" to speed up
K_HEADING = 2.0
OMEGA_MAX = 1.5              # validated fix, confirmed working
...
GPS_NOISE_SIGMA = 0.03           # meters, injected GPS noise (RTK-GPS-ish)
BLACKOUT_START_S = 100.0         # sim time, GPS blackout begins
BLACKOUT_END_S = 140.0           # sim time, GPS blackout ends
GPS_CORRECTION_WEIGHT = 0.4      # complementary filter: weight given to GPS vs dead-reckoning prediction
...
# --- KINEMATICS (real physics-based equations, unchanged) ---
...
while robot.step(TIME_STEP) != -1:
    ...
    gt_x, gt_y = pos[0], pos[1]                     # ground truth (Webots GPS, near-perfect)
    ...
    if est_x is None:  # first sample — initialize estimate at ground truth
        est_x, est_y = gt_x, gt_y
    else:
        # dead-reckoning prediction using last commanded velocity + current yaw
        pred_x = est_x + last_forward_speed * math.cos(yaw) * dt
        pred_y = est_y + last_forward_speed * math.sin(yaw) * dt

        if in_blackout:
            est_x, est_y = pred_x, pred_y  # no GPS correction during blackout
        else:
    ...
    # save for next iteration's dead-reckoning prediction
